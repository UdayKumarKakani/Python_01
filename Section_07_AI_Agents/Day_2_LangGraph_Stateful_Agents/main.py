"""
Day 2 - LangGraph stateful agent (plan -> act -> review + conditional retry)
------------------------------------------------------------------------------
Requires TOGETHER_API_KEY in .env.

Shows:
1. TypedDict state with an operator.add reducer
2. plan / act / review nodes
3. A conditional edge that retries short answers
