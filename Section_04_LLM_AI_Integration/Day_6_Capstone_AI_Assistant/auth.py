...
# Two seeded users so the walkthrough runs out of the box.
_USERS: dict[str, str] = {
...
def register_user(username: str, password: str) -> None:
    """Add a new user to the in-memory store. Raises 409 if the name is taken."""
    if not username or not password:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "username and password required")
    if len(password) < 8:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "password must be >= 8 characters")
