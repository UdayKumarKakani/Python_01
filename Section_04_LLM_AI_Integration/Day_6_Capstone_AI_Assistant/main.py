...
import providers
from auth import authenticate, current_user, issue_token, register_user
...
# Auth
# ---------------------------------------------------------------------------
# NOTE: /auth/token uses OAuth2PasswordRequestForm (application/x-www-form-urlencoded).
# This is the OAuth2 password-flow standard - required for Swagger UI's
# "Authorize" button to work. Send username/password as form fields, NOT JSON:
#   curl -X POST http://localhost:8000/auth/token \
#        -d "username=alice&password=wonderland"
#
# httpx equivalent:  httpx.post(url, data={"username": ..., "password": ...})
@app.post("/auth/token", response_model=TokenResponse)
def login(form: OAuth2PasswordRequestForm = Depends()):
    if not authenticate(form.username, form.password):
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, "bad credentials")
    return TokenResponse(access_token=issue_token(form.username))


@app.post("/auth/register", response_model=TokenResponse, status_code=status.HTTP_201_CREATED)
def register(req: RegisterRequest):
    """Create a new user and return a JWT for immediate use.

    Body (JSON):
        {"username": "carol", "password": "correcthorse"}

    Returns the same token shape as /auth/token so callers can log in immediately.
    """
